    ...
    <a-space direction="vertical" class="relative-child">
      <a-tabs v-model:activeKey="activeKey">
        <a-tab-pane key="mainTab" tab="Dados principais">
          ...
          <a-form-item label="Razão social da empresa" name="companyName">
            <a-input
              :disabled="permission === 'read'"
          ...
          <a-form-item label="Nome fantasia da empresa" name="businessName">
            <a-input
              :disabled="permission === 'read'"
          ...
          <a-form-item label="Data de abertura" name="birthday">
            <a-date-picker
              class="full-width"
              :disabled="permission === 'read'"
          ...
          <a-form-item label="Natureza legal" name="legalNature">
            <a-select
              ref="select"
          ...
          <a-form-item label="Descrição da empresa" name="companyDescription">
            <QuillEditor
              :enable="permission !== 'read'"
              ref="companyDescriptionRef"
              v-model:content="
                companyProfileDynamicValidateForm.companyDescription
              "
          ...
          <div>
            <a-form-item label="Logo" name="logoImage">
              <a-button
                :disabled="permission === 'read'"
          ...
          <div>
            <a-form-item label="Imagens da empresa" name="companyImages">
              <a-button
                :disabled="permission === 'read'"
              ...
              <a-list
                ...
                bordered
                v-if="
                  companyProfileDynamicValidateForm.companyImages?.length > 0
                "
                item-layout="horizontal"
        ...
        <a-tab-pane key="partnersTab" tab="Sócios">
          <a-space
            v-for="(
              partner, partnersIndex
            ) in companyProfileDynamicValidateForm.partners"
            :key="partnersIndex"
            ...
            <a-card class="card">
              ...
              <a-button
                ...
                @click="
                  removePartner(
                    partner,
                    companyProfileDynamicValidateForm.partners
                  )
                "
              >
        ...
        <a-tab-pane key="contactsTab" tab="Contatos">
          <a-space
            ...
            ) in companyProfileDynamicValidateForm.contacts"
            :key="contactsIndex"
            style="display: flex; margin-bottom: 8px"
            align="baseline"
          >
            <a-card class="card">
              <div>
                ...
                <a-form-item label="Contato" name="contact.contactValue">
                  <a-input
                    :disabled="permission === 'read'"
              ...
              <a-button
                ...
                @click="
                  removeContact(
                    contact,
                    companyProfileDynamicValidateForm.contacts
                  )
                "
              >
                <span class="material-symbols-outlined"> delete </span>
          ...
          <div>
            ...
            >
              <a-input
                ...
                @change="
                  onAddressOneCepBrasilApiApiRequestDebounced(
                    addressFieldsetOne.addressOneCepBrasilApi,
                    addressFieldsetOne
                  )
                "
              />
          ...
          <div>
            ...
            >
              <a-input
                ...
                @change="
                  onAddressTwoCepBrasilApiApiRequestDebounced(
                    addressFieldsetTwo.addressTwoCepBrasilApi,
                    addressFieldsetTwo
                  )
                "
              />
        ...
        <a-tab-pane key="relateFilesTab" tab="Arquivos relacionados">
          <a-space
            v-for="(
              relatedFile, relatedFilesIndex
            ) in companyProfileDynamicValidateForm.relatedFiles"
          ...
          >
            <a-card class="card">
              <div>
                ...
                <div>
                  ...
                  >
                    ...
                    <a-list
                      ...
                      bordered
                      v-if="
                        companyProfileDynamicValidateForm.relatedFilesFiles
                          ?.length > 0
                      "
                      item-layout="horizontal"
                      :data-source="
                ...
                <a-form-item
                  :label="
                    'Dia de entrada no trabalho ' + (relatedFilesIndex + 1)
                  "
                ...
                <a-form-item
                  :label="
                    'Mês de entrada no trabalho ' + (relatedFilesIndex + 1)
                  "
              ...
              <a-button
                ...
                @click="
                  removeRelatedFile(
                    relatedFile,
                    companyProfileDynamicValidateForm.relatedFiles
                  )
                "
              >
                <span class="material-symbols-outlined"> delete </span>
          ...
          <a-form-item>
            <a-button
              ...
              @click="
                addRelatedFile(companyProfileDynamicValidateForm.relatedFiles)
              "
            >
              <span class="material-symbols-outlined"> add_circle </span>
...
onMounted(async () => {
  ...
  for (const attr in data) {
    if (attr in companyProfileDynamicValidateForm)
      companyProfileDynamicValidateForm[attr] = data[attr];
    if (attr === "partners" && data.partners) {
      companyProfileDynamicValidateForm.partners = data.partners.map(
        (element: any) => ({
          ...element,
          personId: {
    ...
    if (attr === "tagId" && data.tagId) {
      companyProfileDynamicValidateForm.tagId =
        companyProfileDynamicValidateForm.tagId.map((element: any) => {
          return {
  ...
  if (input.files && input.files[0]) {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3000/companies-profiles/file`,
        formData,
        {
          headers: {
      ...
      companyProfileDynamicValidateForm[attribute] = [
        ...companyProfileDynamicValidateForm[attribute],
        response.data.data,
      ];
      message.success({
        content: "Arquivo carregado com sucesso!",
        key: "upload",
      });
    } catch (error) {
...
const removeFile = (item: any, attribute: string) => {
  companyProfileDynamicValidateForm[attribute] = (
    companyProfileDynamicValidateForm[attribute] as any[]
  ).filter((imageData) => imageData.fileId !== item.fileId);
};
...
const onTagIdSearch = async (searchText: string) => {
  const response = await axios.get(
    `
      ${
        import.meta.env.VITE_BACKEND_ROOT_PATH
      }/tags?filter={"$or":[{"tagName":{"$regex": "${searchText}", "$options": "i"}}]}`,
    {
      headers: {
        Authorization: sessionStorage.getItem("authToken")!,
        "Content-Type": "application/json",
...
const onPersonIdSearch = async (searchText: string) => {
  const response = await axios.get(
    `
      ${
        import.meta.env.VITE_BACKEND_ROOT_PATH
      }/people?filter={"$or":[{"personName":{"$regex": "${searchText}", "$options": "i"}}, {"cpf":{"$regex": "${searchText}", "$options": "i"}}]}`,
    {
      headers: {
        Authorization: sessionStorage.getItem("authToken")!,
        "Content-Type": "application/json",
...
const relatedFilesDateDayRequiredValidator = async (
  _rule: Rule,
  value: any
) => {
  if (!value || value === "") return Promise.reject("Campo obrigatório.");
};
const relatedFilesDateMonthRequiredValidator = async (
  _rule: Rule,
  value: any
) => {
  if (!value || value === "") return Promise.reject("Campo obrigatório.");
};
const relatedFilesDateYearRequiredValidator = async (
  _rule: Rule,
  value: any
) => {
  if (!value || value === "") return Promise.reject("Campo obrigatório.");
};
const relatedFilesDateYearMaxLengthValidator = async (
  _rule: Rule,
  value: any
) => {
  if (value?.length > 4) return Promise.reject("Máximo de 4 caracteres.");
};
const relatedFilesDateYearMinLengthValidator = async (
  _rule: Rule,
  value: any
) => {
...
const companyProfileRules: Record<string, Rule[]> = {
  _id: [{ validator: _idRequiredValidator, trigger: "blur" }],
  cnpj: [],
  companyName: [{ validator: companyNameRequiredValidator, trigger: "blur" }],
  businessName: [{ validator: businessNameRequiredValidator, trigger: "blur" }],
  birthday: [],
  legalNature: [],
  companyDescription: [],
  logoImage: [],
  companyImages: [],
  tagId: [],
  personId: [{ validator: personIdRequiredValidator, trigger: "blur" }],
  contactType: [{ validator: contactTypeRequiredValidator, trigger: "blur" }],
  contactValue: [{ validator: contactValueRequiredValidator, trigger: "blur" }],
  contactComplement: [
    { validator: contactComplementRequiredValidator, trigger: "blur" },
  ],
  filesDescription: [
    { validator: filesDescriptionRequiredValidator, trigger: "blur" },
  ],
  relatedFilesFiles: [
    { validator: relatedFilesFilesRequiredValidator, trigger: "blur" },
  ],
  relatedFilesDateDay: [
    { validator: relatedFilesDateDayRequiredValidator, trigger: "blur" },
  ],
  relatedFilesDateMonth: [
    { validator: relatedFilesDateMonthRequiredValidator, trigger: "blur" },
  ],
  relatedFilesDateYear: [
    { validator: relatedFilesDateYearRequiredValidator, trigger: "blur" },
    { validator: relatedFilesDateYearMaxLengthValidator, trigger: "blur" },
    { validator: relatedFilesDateYearMinLengthValidator, trigger: "blur" },
  ],
};
...
const createCompanyProfile = async () => {
  try {
    if (
      validateCompanyProfileForm?.filter((item) => item === false).length > 0
    ) {
      return message.error({
        content: "Erro no preenchimento do formulário.",
        key: "saveMessage",
      });
    }
    ...
    const payload = {
      ...
      tagId: companyProfileDynamicValidateForm.tagId?.map(
      ...
      partners: companyProfileDynamicValidateForm.partners.map(
        (partner: any) => {
          return {
            ...
            personId:
              typeof partner.personId === "object"
...
const updateCompanyProfile = async (id: string) => {
  try {
    ...
    message.error({
      content: "Erro ao atualizar os dados.",
      key: "saveMessage",
    });
    console.error(e.message);
  }
