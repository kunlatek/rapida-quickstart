  ...
  onMount(async () => {
    ...
    try {
      ...
      if (profiles.hasCompany) {
        toastStore.warning("Você já possui um perfil de empresa.");
        goto(`/profile/company/${profiles.companyId}`);
        return;
      }
    } catch (error) {
      console.error("Erro ao verificar perfil:", error);
      // Continuar mesmo com erro, para permitir criação do perfil
    } finally {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (response && response.access_token) {
        ...
        try {
          const switchSuccess = await authService.switchRole("company");
          if (switchSuccess) {
  ...
  <div class="max-w-4xl mx-auto pb-20">
    <Card>
      <Heading tag="h1" class="text-2xl font-bold mb-6"
        >Criar Perfil de Empresa</Heading
      >
      <div class="text-gray-500 dark:text-gray-400 mb-6">
        <p>
          Preencha o formulário abaixo para criar seu perfil de empresa. Os
